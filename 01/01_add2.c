 ...
 */
int findNum2 ( int data[], int nr, int val, int * mulRes )
{
...
/*-----------------------------------------------------------------------------------------------*/
/**
 * Find three numbers which add up to *val* and return their product. If there are multiple such
 * tuples, the first one is returned.
 * @param[in]  data    list of numbers
 * @param[in]  nr      length
 * @param[in]  val     value the pair ought to add up to
 * @param[out] mulRes  product of the pair
...
int findNum ( int data[], int nr, int val, int * mulRes )
{
  ...
  {
    seek = val - data[i];
    if ( findNum2 ( data + i, nr - i, seek, mulRes ) )
    {
     *mulRes *= data[i];
      return 1;
    }
...
int main ( int argc, char * argv [] )
{
  ...
  {
    printf ( "No such tuple of numbers found\n" );
    return 0;
  }
